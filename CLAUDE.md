...
- **HTTP-based inter-service communication**: All services communicate via REST APIs
- **Service Discovery**: Automatic adaptation between Docker Compose (container names) and local development (localhost:ports)
- **Real-time Updates**: Socket.IO for frontend notifications (crawl progress, task updates)
- **MCP Protocol**: Server-Sent Events (SSE) transport for AI client connections
- **Volume Mounts**: Live code reloading in Docker containers via volume mounts

## Development Commands

### Quick Start/Restart
```bash
# Automated daily startup (recommended)
./scripts/good-morning.sh    # Complete Docker restart with health checks
good-morning                 # Alias version (after running ./scripts/good-morning-alias.sh)
```
...
npm run seed:projects    # Seed test project data
```
...
uv run ruff check        # Lint code (line length: 120)
...
docker-compose down && docker-compose up --build -d  # Full restart
```
...
- **Node.js**: Standard npm with package.json, supports Node 18+
- **Container Dependencies**: All services defined in docker-compose.yml with health checks
- **Environment Variables**: Configured via `.env` file, see Environment Variables section
...
For daily development or recovering from connection issues, use the automated good-morning command:

```bash
./scripts/good-morning.sh
```

This handles Docker startup, service restart, and health validation automatically.
